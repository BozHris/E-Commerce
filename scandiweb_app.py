...
# Define the weighted mean squared error loss function
def weighted_mse(y_true, y_pred):
    weights = K.cast(K.greater(y_true, 1), dtype='float32') + 1
    return K.mean(weights * K.square(y_pred - y_true), axis=-1)

# Load models
try:
    classification_model = load_model('scandiweb_binary_prediction_3.h5')
    regression_model = load_model('scandiweb_regression_prediction_two.h5', custom_objects={'weighted_mse': weighted_mse})
except Exception as e:
    st.error(f"Error loading models: {e}")
    st.stop()

# Define the categories and channels
item_categories = [
